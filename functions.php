...
function loginStud(){
	...
	$recruiter = $bdd->query("SELECT * FROM etudiant where email_etudiant='$login'" );
...
function loginProf(){
	...
	$recruiter = $bdd->query("SELECT * FROM professeur where email_prof='$login' ");
	foreach ($recruiter->fetchall() as $data) {
...
function getPourcentageOfTestAlreadyPassed($id_cours){
	global $bdd;
	...
	$pourcentage = 0;
	while ($r = $q->fetch()) {
		$pourcentage = $pourcentage + $r['pourcentage_test'];
	}

	return json_encode(['nbrEpreuve' => $count, 'pourcentageDone' => $pourcentage]);
}
...
function addNoteStudentToDatabase($selectedCourse){
	...
	while($r = $q->fetch()){
		...
		$noteOfThatStudent = $_GET['noteStud'.$tmp];
...
function getNotesOfEtudiant($code_referentiel){
	...
	foreach ($tableau_general['notes'][0] as $note) {
		...
		if ($tableau_general['elements_modules'][0][$i]==$module) {
			...
			if ($i< count($tableau_general['notes'][0])-1) {
				
				if ($module != $tableau_general['elements_modules'][0][$i+1]){
					$module = $tableau_general['elements_modules'][0][$i+1];
				}
			}
		...
		if ($tableau_general['elements_modules'][0][$i]!= $module || $i== count($tableau_general['notes'][0])-1) {
			...
			array_push($tableau_coeff,$tableau_general['coeff_element_module'][0][$i]);
